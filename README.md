...
### 🎨 命令式功能
   - **风格转换**：`/cp <风格>` - 快速应用预设风格（仅图生图）
   - **自然语言**：`/cp 画一只猫` - 智能判断文/图生图
   - **模型指定**：`/cp 用model1画猫` - 自然语言中动态指定模型
   - **模型管理**：`/cp list`、`/cp set <模型ID>` - 动态切换模型
   - **风格管理**：`/cp styles`、`/cp style <风格名>` - 查看风格详情
...
2. **模型配置管理**
   - `/cp list` - 查看所有可用模型
   - `/cp set <模型ID>` - 动态切换图生图命令使用的模型
   - `/cp config` - 查看当前配置
   - `/cp reset` - 重置为默认配置

3. **管理员命令**（需在admin_users中配置）
   - `/cp on` / `/cp off` - 启用/禁用当前聊天的插件
   - `/cp model on|off <模型ID>` - 启用/禁用指定模型
   - `/cp recall on|off <模型ID>` - 启用/禁用指定模型的自动撤回
   - `/cp default <模型ID>` - 设置Action组件默认模型
...
- 🛠️ **管理员命令**：新增 `/cp on|off`、`/cp model`、`/cp recall`、`/cp default`

### v3.2.0
- 🎯 **新增自拍模式**：支持生成Bot角色的自拍照片，包含40+种智能手部动作库
- 🎨 **自然语言命令**：`/cp 画一只猫` 智能判断文/图生图
- 🔧 **模型指定**：支持 `/cp 用model1画猫` 动态指定模型
